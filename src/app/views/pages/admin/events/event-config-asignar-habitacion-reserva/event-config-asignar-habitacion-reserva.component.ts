...
@Component({
  selector: 'app-event-config-asignar-habitacion-reserva',
  templateUrl: './event-config-asignar-habitacion-reserva.component.html',
  styleUrls: ['./event-config-asignar-habitacion-reserva.component.scss'],
})
export class EventConfigAsignarHabitacionReservaComponent implements OnInit {
  ...
  saveConfigHabitaciones() {
    this.alertsService.confirm({
      ...
      message:
        '¿Está seguro de asignar la habitación? Esta acción no se puede deshacer.',
      okCallback: () => {
        this.eventoService
          .asignarHabitacionReservada({
            eventoId: this.eventoId!.toString(),
            tipoHabitacionId:
              this.habitacionAReservar.tipoHabitacionId.toString(),
